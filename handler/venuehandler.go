...
func GetVenues() (int, string) {
	venues, err := repository.GetVenues(); if err != nil {
...
func PutVenue(ctx *macaron.Context) (int, string) {
	...
	venue, err := repository.AddVenue(name, image); if err != nil {
...
func PostVenue(ctx *macaron.Context) (int, string) {
	...
	// image
	if data["image"] != nil {
		image, ok := data["image"].(string); if !ok {
	...
	err = repository.UpdateVenue(key, &values); if err != nil {
...
func DeleteVenue(ctx *macaron.Context) (int, string) {
	...
	err := repository.DeleteVenue(key); if err != nil {
